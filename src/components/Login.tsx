          ...
          {(['worker', 'counselor', 'admin'] as const).map(r => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-200 ${
                role === r 
                  ? 'bg-[#002c5f] text-[#38bdf8] border border-[#38bdf8]/40 shadow-[0_0_12px_rgba(14,165,233,0.2)]' 
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          {role === 'worker' && (
            <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
              <div>
                <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">{role === 'worker' ? t('login.emp_id') : '사번 (Employee ID)'}</label>
                <input
                  type="text"
                  value={uid}
                  maxLength={7}
              ...
              <div>
                <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">{role === 'worker' ? t('login.password') : '비밀번호 숫자 4자리 (4-Digit Password)'}</label>
                <input
                  type="password"
                  inputMode="numeric"
                  pattern="[0-9]*"
          ...
          {role === 'counselor' && (
            <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
              <div>
                <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">통역위원 ID</label>
                <input
                  type="text"
              ...
              {isChangingPassword && (
                <div className="space-y-4 animate-fade-in-up mt-4 pt-4 border-t border-[#1e3a5f]">
                  <div>
                    <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">새 비밀번호</label>
                    <input
                      type="password"
                      value={newPassword}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">새 비밀번호 확인</label>
                    <input
                      type="password"
              ...
              <div className="flex justify-end mt-2">
                <button
                  ...
                  className="text-xs text-[#38bdf8] hover:text-[#7dd3fc] transition-colors flex items-center gap-1 font-semibold"
                >
          ...
          {role === 'admin' && (
            <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
              <div>
                <label className="block text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-2">관리자 ID</label>
                <input
                  type="text"
                  value={counselorInputId}
                  onChange={e => setCounselorInputId(e.target.value)}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="dx-btn-primary w-full py-3.5 mt-6 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_14px_rgba(0,168,89,0.3)]"
          >
